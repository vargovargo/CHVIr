...
workingPath <- "//phdeorlcsrvip01/Crossbranch/CDC_BRACE/CHVI-CHPR Data/data/" # work local

###  filenames ### 
wildfire <- "https://www.cdph.ca.gov/Programs/OHE/CDPH%20Document%20Library/CHVIs/BRACE_Wildfire_786_CT_PL_CO_RE_CA.xlsx"
slr <- "https://www.cdph.ca.gov/Programs/OHE/CDPH%20Document%20Library/CHVIs/BRACE_SLR_784_CT_PL_CO_RE_CA_11-1-2016.xlsx"
ozone <- "https://www.cdph.ca.gov/Programs/OHE/CDPH%20Document%20Library/CHVIs/BRACE_Ozone_801_CT_PL_CO_RE_CA.XLSX"
pm <- "https://www.cdph.ca.gov/Programs/OHE/CDPH%20Document%20Library/CHVIs/BRACE_PM25levels_776_CT_PL_CO_RE_CA.XLSX"
heat <- "https://www.cdph.ca.gov/Programs/OHE/CDPH%20Document%20Library/CHVIs/BRACE_ExtremeHeat_791_CO.xlsx"
...
poverty <-"~/CHVI_copy/HCI_PovertyRate_754_CT_PL_CO_RE_CA_1-22-14.xlsx"
education <-"~/CHVI_copy/ed_attain_ge_hs_output04-14-13.xlsx"
canopy <- "https://www.cdph.ca.gov/Programs/OHE/CDPH%20Document%20Library/CHVIs/BRACE_TreeCanopy_458_CT_PL_CO_RE_CA.xlsx"
impervious <-"https://www.cdph.ca.gov/Programs/OHE/CDPH%20Document%20Library/CHVIs/BRACE_ImperviousSurfaces_423_CT_PL_CO_RE_CA.xlsx"
ac <- "https://www.cdph.ca.gov/Programs/OHE/CDPH%20Document%20Library/CHVIs/BRACE_AirConditioning_797_CO_RE_CA.xlsx"
...
simplifyCHVI <- function(indicator = "indicator"){
   ...
   names(temp2) <- c("def", "cnty", "geotypv", "geotype", "est", "region", "LL95", "UL95", "numratr", "denmntr", "ct10", "strata", "ind")
   
   temp2 %>% write.csv(paste0(workingPath,"tables/tracts/CHVI_",indicator,"_tract.csv"),row.names=F) # it writes a file to your home directory with the name including the indicator and the geography
   ...
   temp2 %>% write.csv(paste0(workingPath,"tables/counties/CHVI_",indicator,"_county.csv"),row.names=F) # it writes a file to your home directory with the name including the indicator and the geography
...
indicators <- c("wildfire","ozone","pm","SLR", "children","elderly","vehicles","outdoor")
...
# run it for pop weigthed imperv
strat <- 'population-weighted' # use population-weighted or area-weighted for imperv

# parse and write pop-weighted TRACT data
temp2 <- temp %>% 
  filter(geotype == "CT", race_eth_name == "Total", strata_one_name == strat)  %>% # here's where we can filter the data further
  select(ind_definition, county_name, geotypevalue, geotype, estimate, region_name, LL_95CI, UL_95CI, numerator, denominator) %>% # the file is parsed down to these columns only
  mutate(ct10= as.character(geotypevalue),
         strata = strat, 
         ind=indicator)
...
temp2 %>% write.csv(paste0(workingPath,"tables/tracts/CHVI_",indicator,"_",strat,"_tract.csv"),row.names=F) # it writes a file to your home directory with the name including the indicator and the geography

# parse and write pop-weighted COUNTY data
temp2 <- temp %>% 
  filter(geotype == "CO", race_eth_name == "Total", strata_one_name == strat)  %>% # here's where we can filter the data further
  select(ind_definition, county_name, geotypevalue, geotype, estimate, region_name, LL_95CI, UL_95CI, numerator, denominator) %>% # the file is parsed down to these columns only
  mutate(COUNTYFI_1= as.character(geotypevalue), 
         strata = strat, 
         ind=indicator) 
...
strat <- 'area-weighted' # use population-weighted or area-weighted for imperv
...
strat <- 'population-weighted' # use population-weighted or area-weighted for canopy
...
strat <- 'area-weighted' # use population-weighted or area-weighted for canopy
...
# run it for pop weigthed imperv
strat <- 'All Non-English' # use population-weighted or area-weighted for imperv
...
p1f <- tempfile()
download.file(fileName, p1f, mode="wb")
temp <- readxl::read_xlsx(p1f, sheet="ViolentCrime") %>% select(1:26) %>% .[,order(names(.))] 

temp$reportyear <- as.character(as.numeric(temp$reportyear))

temp2 <- temp %>% 
  filter(geotype == "CO", race_eth_name == "Total", strata_level_name_code == 5, reportyear == 2013)  %>% # here's where we can filter the data further
  select(ind_definition, county_name, geotypevalue, geotype, rate, region_name, ll_95ci, ul_95ci, numerator, denominator) %>% # the file is parsed down to these columns only
  mutate(COUNTYFI_1= as.character(geotypevalue),
         strata= "ViolentCrime",
         ind= indicator) 
...
temp2 <- temp %>% 
  filter(geotype == "CT", race_eth_name == "Total", Poverty == strat, reportyear == "2006-2010")  %>% # here's where we can filter the data further
...
# parse and write TRACT data
temp2 <- temp %>% 
  filter(geotype == "CT", race_eth_name == "Total", reportyear == "2006-2010")  %>% # here's where we can filter the data further
  select(ind_definition, county_name, geotypevalue, geotype, p_hs_edatt, region_name, LL_95CI, UL_95CI, pop25pl_hs, pop25pl) %>% # the file is parsed down to these columns only
  mutate(ct10= as.character(geotypevalue),
         strata = "2006-2010", 
         ind=indicator)
...
# parse and write COUNTY data
temp2 <- temp %>% 
  filter(geotype == "CO", race_eth_name == "Total", reportyear == "2006-2010")  %>% # here's where we can filter the data further
  select(ind_definition, county_name, geotypevalue, geotype, p_hs_edatt, region_name, LL_95CI, UL_95CI, pop25pl_hs, pop25pl) %>% # the file is parsed down to these columns only
  mutate(COUNTYFI_1= as.character(geotypevalue), 
         strata = "2006-2010",
         ind=indicator) 
...
#############################
### combine all the file for COUNTY level
...
workingPath <- "~/CHVI_copy/data/" # work local
workingPath <- "//phdeorlcsrvip01/Crossbranch/CDC_BRACE/CHVI-CHPR Data/data/" # work on network
...
for(file in files){
  temp <- read.csv(file, header=T, stringsAsFactors = FALSE) %>%
    mutate(numratr = ifelse(class(numratr) == "logical",as.numeric(as.character(numratr)),numratr),
           denmntr = ifelse(class(denmntr) == "logical",as.numeric(as.character(denmntr)),denmntr),
           strata = ifelse(class(strata) == "integer",as.character(as.numeric(strata)),strata))
...
countyTab <- countyCHVIs %>%
  ...
  gather(est, LL95, UL95, numratr, denmntr,key = metric, value = value) %>% 
  select(-region) %>%
  left_join(read.csv(url("https://raw.githubusercontent.com/vargovargo/CHVIr/master/county_region_lookup.csv"), header=TRUE)) 

countyTab %>% write.csv(paste0(workingPath,"tables/CHVIcountiesTIDY.csv"), row.names= FALSE)
...
names(countyShpTab) <- c("county", "COUNTYFI_1","AC","arTREE","popTREE","CHILD","CRIME","mntlDSBL","physDSBL",           
                         "totDSBL","EDU", "ELDER", "HEAT50","HEAT85", "arIMPRV","popIMPRV","INSUR","LING",
                         "OUTDR","OZN","PM","chldPOV","concPOV",  "allPOV","SLR","VEH","WF", "ClimReg")

counties <- st_read(paste0(workingPath,"shapefiles/Counties2010.shp"))

CHVIcounties <- counties %>%  left_join(countyShpTab) %>% st_transform(crs = 4326) 

plot(CHVIcounties)

CHVIcounties %>%
  st_write(dsn = normalizePath(paste0(workingPath,"shapefiles/CHVI_indicators_county.shp")), delete_layer=TRUE)
...
names(tractShpTab) <- c("county", "ct10","arTREE","popTREE","CHILD","mntlDSBL","physDSBL",           
                         "totDSBL","EDU", "ELDER", "arIMPRV","popIMPRV","INSUR","LING",
                         "OUTDR","OZN","PM","chldPOV","allPOV","SLR","VEH","WF", "ClimReg")

tracts <- st_read(paste0(workingPath,"shapefiles/census_tracts2010.shp"))

CHVItracts <- tracts %>%  left_join(tractShpTab) %>% st_transform(crs = 4326) 

plot(CHVItracts)

CHVItracts %>%
  st_write(dsn = normalizePath(paste0(workingPath,"shapefiles/CHVI_indicators_tract.shp")), delete_layer=TRUE)
